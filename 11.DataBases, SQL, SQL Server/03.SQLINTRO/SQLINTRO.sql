...
SELECT DISTINCT Salary AS [All Distinct Salaries] 
FROM Employees
...
SELECT e1.FirstName + ' ' + e1.LastName AS Name, e2.FirstName + '' + e2.LastName AS Manager
FROM Employees e1
JOIN Employees e2
...
SELECT e.FirstName + ' ' + e.LastName AS Name, m.FirstName + '' + m.LastName AS Manager, a.AddressText AS Address
FROM Employees e
JOIN Employees m
...
WHERE d.Name IN ('Sales', 'Finance') AND (YEAR(e.HireDate) >= 1995 AND YEAR(e.HireDate) <= 2005)
